...
const AddTractor = () => {
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      await api.post('/tractor', payload);
      toast.success('Tractor registered successfully!');
      navigate('/tractors');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to register tractor');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="p-4 sm:p-6 lg:p-8 max-w-4xl mx-auto">
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 sm:p-10">
        <header className="mb-8 border-b border-slate-100 pb-6">
          ...
          <p className="text-slate-500 mt-2">List your tractor for rent and earn money</p>
        </header>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-8">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            <div>
              <label className="block text-sm font-medium text-slate-700 mb-1.5">Tractor Type</label>
              <input 
                className={`w-full px-3 py-2 border rounded-lg text-sm focus:ring-2 focus:ring-sky-500 focus:outline-none ${errors.tractorType ? 'border-red-300 bg-red-50' : 'border-slate-300'}`}
                placeholder="e.g. Cultivator, Rotavator"
                {...register('tractorType', { required: true })} 
              />
              {errors.tractorType && <p className="mt-1 text-sm text-red-600">Tractor type is required</p>}
            </div>
            ...
            <div>
              ...
              <input 
                ...
                placeholder="e.g. Mahindra, Swaraj"
                {...register('brand')} 
              />
            </div>
          ...
          <div className="pt-4 border-t border-slate-100">
            <h3 className="text-lg font-bold text-slate-900 mb-4">Upload Tractor Images</h3>
          ...
          <div className="pt-6 flex justify-end">
            ...
            >
              {loading ? 'Registering...' : 'Register Tractor'}
            </Button>
          </div>
        </form>
      </div>
