...
const JCBSchema = new mongoose.Schema({
  owner: { type: mongoose.Schema.Types.ObjectId, ref: 'User', required: true, index: true },
  jcbType: { type: String, required: [true, 'JCB type is required'], trim: true },
...
JCBSchema.index({ status: 1, isAvailable: 1, state: 1, district: 1, block: 1, village: 1, ratePerHour: 1, rating: -1 });
JCBSchema.index({ location: '2dsphere' });

module.exports = mongoose.model('JCB', JCBSchema);
